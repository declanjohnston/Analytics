...
def cf(player_id,events):
    ...
    filtered_events = list(filter(lambda x: (((x[GLOBAL.EVENT_INDEX] == "SHOT") or (x[GLOBAL.EVENT_INDEX] == "MISS") or (x[GLOBAL.EVENT_INDEX] =="BLOCK") or (x[GLOBAL.EVENT_INDEX] =="GOAL")) and x[11] == team(player_id,x) and x[GLOBAL.PERIOD_INDEX] != str(5) and event_info.is_5v5(x)), events))
...
def gf(player_id, events):
    ...
    filtered_events = list(filter(lambda x: (x[GLOBAL.EVENT_INDEX] == "GOAL" and x[GLOBAL.EVENT_TEAM_INDEX] == team(player_id,x) and x[GLOBAL.PERIOD_INDEX] != str(5) ), events))
    return GF
